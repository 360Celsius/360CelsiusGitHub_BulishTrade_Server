...
for ($x = 0; $x <= (count($arrayFiltered_27)-1)/2 ; $x++) {
  
    $url = 'https://api.iextrading.com/1.0/stock/'. trim($arrayFiltered_27[$x]) .'/news/last/1';
    $ch = curl_init($url);
    ...
    for($y = 0; $y <= count($result)-1; $y++){
        ...
        $sql = "REPLACE INTO news_data_27 (id,news_datetime,headline,news_source,url,summary,related,image)
